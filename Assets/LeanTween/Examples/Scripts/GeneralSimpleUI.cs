...
public class GeneralSimpleUI : MonoBehaviour {
	...
	void Start () {
		GameLog.LogMessage("For better examples see the 4.6_Examples folder!");
		if(button==null){
			GameLog.LogError("Button not assigned! Create a new button via Hierarchy->Create->UI->Button. Then assign it to the button variable");
			return;
		}
		...
		LeanTween.value(gameObject, 1f, 0.5f, 1f ).setOnUpdate( 
			(float volume)=>{
				GameLog.LogMessage("volume:"+volume);
			}
		);
	...
	#else
	void Start(){
		GameLog.LogMessageError("Unity 4.6+ is required to use the new UI");
	}
