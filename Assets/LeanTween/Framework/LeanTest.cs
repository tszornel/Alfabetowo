...
public class LeanTester : MonoBehaviour {
	...
	IEnumerator timeoutCheck(){
		...
		if(LeanTest.testsFinished==false){
			GameLog.LogMessage(LeanTest.formatB("Tests timed out!"));
			LeanTest.overview();
		}
	}
	#endif
...
public class LeanTest : object {
	...
	public static void expect( bool didPass, string definition, string failExplaination = null){
		...
		GameLog.LogMessage(logName);
		if(didPass)
			passes++;
		tests++;
		
		// GameLog.LogMessage("tests:"+tests+" expected:"+expected);
		if(tests==expected && testsFinished==false){
			overview();
		}else if(tests>expected){
			GameLog.LogMessage(formatB("Too many tests for a final report!") + " set LeanTest.expected = "+tests);
		}
	...
	public static void overview(){ 
		...
		GameLog.LogMessage(formatB("Final Report:")+" _____________________ PASSED: "+formatBC(""+passes,"green")+" FAILED: "+failedStr+" ");
	}
}
