...
namespace Webaby.Utils {
    ...
    public class World_Bar {
        ...
        private void SetupOutline(Outline outline, Vector3 localScale, int sortingOrder) {
            UtilsClass.CreateWorldSprite(transform, "Outline", WebabyAssets.i.s_White, new Vector3(0,0), localScale + new Vector3(outline.size, outline.size), sortingOrder, outline.color);
        }
        private void SetupBackground(Color backgroundColor, Vector3 localScale, int sortingOrder) {
            background = UtilsClass.CreateWorldSprite(transform, "Background", WebabyAssets.i.s_White, new Vector3(0,0), localScale, sortingOrder, backgroundColor).transform;
        }
        private void SetupBar(Color barColor, Vector3 localScale, int sortingOrder) {
            ...
            Transform barIn = UtilsClass.CreateWorldSprite(bar, "BarIn", WebabyAssets.i.s_White, new Vector3(localScale.x / 2f, 0), localScale, sortingOrder, barColor).transform;
        }
